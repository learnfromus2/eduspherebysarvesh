...
CONVERSATION CONTEXT: Maintain continuity and build upon previous discussions to create a comprehensive learning experience.`;
  ...
  app.get('/api/questions', async (req, res) => {
    try {
      const { examType, subject, difficulty, chapter, class: classLevel } = req.query;
      let query = {};
      ...
      console.log(`✅ Found ${questions.length} questions for ${examType} ${subject}`);
  ...
  <div class="header">
    ...
    <div class="header-info">
      <div class="header-left">
        ...
        <div><strong>Total Mark</strong> : ${totalMarks}</div>
      </div>
      <div class="header-center">
        <div style="margin-bottom: 3px;">${paperTitle}</div>
        ${typeLabel ? `<div style="font-size: 10pt;">${typeLabel}</div>` : ''}
      </div>
      <div class="header-right" style="text-align: right;">
        <div><strong>Paper Set</strong> : ${paperSet}</div>
        <div><strong>Date</strong> : ${examDate}</div>
        <div><strong>Time</strong> : ${examTime}</div>
      </div>
    </div>
  </div>
`;
  ...
  app.get('/', (req, res) => {
    ...
    res.setHeader('Pragma', 'no-cache');
    res.setHeader('Expires', '0');
    res.sendFile(path.join(__dirname, 'mine', 'index.html'));
  });
